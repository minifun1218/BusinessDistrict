...
from .clients.dianping_client import DianpingClient
...
class DataSourceManager:
    ...
    def __init__(self, baidu_api_key: str = None, amap_api_key: str = None):
        ...
        self.clients = {}
        self.stats = {
            'total_areas_fetched': 0,
            'total_stores_fetched': 0,
    ...
    def _init_clients(self):
        ...
        try:
            if self.baidu_api_key:
                self.clients['baidu'] = BaiduMapClient(self.baidu_api_key)
                logger.info("百度地图API客户端初始化成功")
            
            if self.amap_api_key:
                self.clients['amap'] = AmapClient(self.amap_api_key)
                logger.info("高德地图API客户端初始化成功")
            
            # 大众点评模拟客户端
            self.clients['dianping'] = DianpingClient()
            logger.info("大众点评模拟客户端初始化成功")
            
            logger.info(f"数据源管理器初始化完成，共加载 {len(self.clients)} 个客户端")
            
        except Exception as e:
            logger.error(f"数据源客户端初始化失败: {str(e)}")
            raise
    
    def fetch_city_data(self, city_id: str, city_name: str, 
        ...
        Args:
            city_id: 城市ID
            city_name: 城市名称
            data_sources: 指定使用的数据源列表，None表示使用所有可用数据源
        ...
        """
        try:
            logger.info(f"开始获取城市 {city_name}({city_id}) 的数据")
            
            # 确定要使用的数据源
            active_sources = data_sources or list(self.clients.keys())
            active_sources = [name for name in active_sources if name in self.clients]
            
            if not active_sources:
                logger.error("没有可用的数据源")
                return {'success': False, 'error': '没有可用的数据源'}
            ...
            with ThreadPoolExecutor(max_workers=len(active_sources)) as executor:
                future_to_source = {
                    executor.submit(
                        self.clients[source_name].get_business_areas,
                        city_id, city_name
                    ): source_name
                ...
                for future in as_completed(future_to_source):
                    ...
                    try:
                        areas = future.result()
                        logger.info(f"{source_name} 数据源获取到 {len(areas)} 个商圈")
                        all_areas.extend(areas)
                    except Exception as e:
                        logger.error(f"{source_name} 数据源执行失败: {str(e)}")
            ...
            for area in saved_areas:
                stores_count = self._fetch_area_stores(
                    area['id'], area['name'], 
                    area['latitude'], area['longitude'],
                    active_sources, update_existing
                )
                total_stores += stores_count
            
            # 更新统计信息
            self.stats['total_areas_fetched'] += len(saved_areas)
            self.stats['total_stores_fetched'] += total_stores
            ...
            logger.info(f"城市 {city_name} 数据获取完成: {result}")
            return result
            
        except Exception as e:
            logger.error(f"获取城市 {city_name} 数据失败: {str(e)}")
            return {
                'success': False,
                'city_id': city_id,
                'city_name': city_name,
    ...
    def _fetch_area_stores(self, area_id: str, area_name: str, 
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=len(source_names)) as executor:
                future_to_source = {
                    executor.submit(
                        self.clients[source_name].get_stores,
                        area_id, area_name, area_lat, area_lng
                    ): source_name
                    for source_name in source_names
                    if hasattr(self.clients[source_name], 'get_stores')
                }
                ...
                for future in as_completed(future_to_source):
                    ...
                    try:
                        ...
                        logger.info(f"{source_name} 数据源为商圈 {area_name} 获取到 {len(stores)} 个店铺")
                        all_stores.extend(stores)
                    except Exception as e:
                        logger.error(f"{source_name} 数据源获取商圈 {area_name} 店铺失败: {str(e)}")
    ...
    def _merge_area_data(self, areas: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for area in areas:
            ...
            if key not in merged_areas:
                ...
                if area.get('avg_consumption', 0) > existing.get('avg_consumption', 0):
                    existing['avg_consumption'] = area['avg_consumption']
    ...
    def _merge_store_data(self, stores: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for store in stores:
            ...
            if key not in merged_stores:
                ...
                # 取更合理的价格（非零值）
                if store.get('avg_price', 0) > 0 and existing.get('avg_price', 0) == 0:
                    existing['avg_price'] = store['avg_price']
                
                # 合并联系信息
                if store.get('phone') and not existing.get('phone'):
