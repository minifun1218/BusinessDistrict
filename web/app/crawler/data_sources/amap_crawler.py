...
class AmapCrawler(BaseCrawler):
    ...
    def get_stores(self, area_id: str, area_name: str, area_lat: float = None, area_lng: float = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for poi_type in poi_types:
                type_stores = self._search_around(
                    location=f"{area_lng},{area_lat}",
                    types=poi_type,
                    radius=2000  # 2公里范围
                )
                stores.extend(type_stores)
    ...
    def _search_places(self, keywords: str, city: str, types: str = None) -> List[Dict[str, Any]]:
        """搜索地点"""
        try:
    ...
    def _search_around(self, location: str, types: str, radius: int = 1000) -> List[Dict[str, Any]]:
        """周边搜索"""
        try:
    ...
    def _format_business_area(self, area: Dict[str, Any], city_id: str, city_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'transportation': self._extract_transportation(area),
    ...
    def _format_store(self, store: Dict[str, Any], area_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'longitude': lng,
                'latitude': lat,
                'rating': self._extract_rating(store),
                'review_count': 0,  # 高德API通常不提供评论数
                ...
                'images': self._extract_images(store),
    ...
    def _determine_area_type(self, name: str, poi_type: str) -> str:
        """判断商圈类型"""
        if '购物' in name or '商场' in name or '百货' in name or 'mall' in name.lower():
            return 'shopping'
        elif '美食' in name or '餐饮' in name or '小吃' in name:
            return 'dining'
        elif '娱乐' in name or '休闲' in name or '酒吧' in name:
    ...
    def _determine_area_level(self, area: Dict[str, Any]) -> str:
        ...
        # 根据知名度判断级别
        if any(keyword in name for keyword in ['万达', '银泰', '大悦城', '龙湖', '华润', '恒隆']):
            return 'A'
        elif any(keyword in name for keyword in ['购物中心', '广场', '商城']):
            return 'B'
        else:
            return 'C'
    ...
    def _calculate_hot_value(self, area: Dict[str, Any]) -> int:
        ...
        # 根据地址判断位置重要性
        address = area.get('address', '')
        if any(keyword in address for keyword in ['市中心', 'CBD', '核心区']):
            base_score += 15
    ...
    def _estimate_customer_flow(self, area: Dict[str, Any]) -> int:
        ...
        # 根据商圈热度估算日均客流量
        hot_value = self._calculate_hot_value(area)
        return int(hot_value * 1000)  # 简单的线性关系
    
    def _extract_rating(self, store: Dict[str, Any]) -> float:
        """提取评分"""
        # 高德API通常不直接提供评分，可以设置默认值或从其他字段推断
        return 4.0  # 默认评分
    ...
    def _extract_transportation(self, area: Dict[str, Any]) -> List[Dict[str, Any]]:
        """提取交通信息"""
        # 这里可以调用高德的公交API获取交通信息
        # 暂时返回空列表
        return []
