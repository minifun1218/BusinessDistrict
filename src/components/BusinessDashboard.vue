<template>
  ...
  const initMainMap = () => {
    ...
    const option = {
      ...
      tooltip: {
        ...
        backgroundColor: 'rgba(10, 18, 25, 0.9)',
        borderColor: '#8c9eff',
        textStyle: {
          color: '#fff'
        }
  ...
  const handleAreaSelected = async (area) => {
    ...
    // 检查数据库中是否有爬取记录，如果有直接加载analytics数据，否则先爬取
    await checkAndLoadAreaData(area)
  }
  ...
  // 检查并加载商圈数据
  const checkAndLoadAreaData = async (area) => {
    if (!area) return

    try {
      console.log('检查商圈数据:', area.name)
      
      // 先检查数据库中是否有该商圈的爬取记录
      const checkResponse = await businessApi.checkAreaDataExists(area.id || area.name)
      
      if (checkResponse.exists && checkResponse.data) {
        console.log('数据库中已有商圈数据，直接加载analytics数据')
        // 直接加载analytics数据
        await updateChartsForArea(area)
        
        showNotification(`已加载 ${area.name} 的缓存数据`, 'success')
      } else {
        console.log('数据库中无商圈数据，开始爬取...')
        
        // 显示爬取状态
        showNotification(`正在爬取 ${area.name} 的详细数据...`, 'info')
        
        // 爬取商圈详细数据
        const response = await businessApi.crawlAreaDetails(area.id || area.name)
        
        if (response && response.data) {
          console.log('爬取商圈数据成功，保存到数据库并加载analytics数据')
          
          // 更新选中商圈的数据
          selectedArea.value = {
            ...selectedArea.value,
            ...response.data.area,
            store_statistics: response.data.store_statistics,
            reviews_sample: response.data.reviews_sample,
          ...
          showNotification(
            response.data.cached ?
              `从缓存获取 ${area.name} 详细数据` :
              `成功爬取 ${area.name} 最新数据并保存`,
            'success'
          )
          
          // 触发area-data-updated事件（用于兼容现有逻辑）
      ...
      console.error('检查和加载商圈数据失败:', error)
      showNotification(`加载 ${area.name} 数据失败: ${error.message}`, 'error')
    }
  }
  ...
  const updateChartsForArea = async (area) => {
    ...
    try {
      // 加载特定商圈的详细数据
      await Promise.all([
        loadAreaHotRankingData(area.name),
        loadAreaHourlyFlowData(area.name),
        loadAreaCategoryData(area.name),
        loadAreaSentimentData(area.name),
        loadAreaTrendData(area.name),
        loadAreaRadarData(area.name)
      ])
  ...
  // 加载特定商圈的热度排行数据
  const loadAreaHotRankingData = async (areaName) => {
    try {
      const data = await analyticsApi.getAreaHotRankingData(areaName, selectedCity.value.id)
  ...
  // 加载特定商圈的客流数据
  const loadAreaHourlyFlowData = async (areaName) => {
    try {
      const data = await analyticsApi.getAreaHourlyFlowData(areaName, selectedCity.value.id)
      if (data) {
        // 更新客流图表
  ...
  // 加载特定商圈的类型分布数据
  const loadAreaCategoryData = async (areaName) => {
    try {
      const data = await analyticsApi.getAreaCategoryDistribution(areaName, selectedCity.value.id)
      if (data && data.length > 0) {
        // 更新类型分布图表
  ...
  // 加载特定商圈的情感分析数据
  const loadAreaSentimentData = async (areaName) => {
    try {
      const data = await analyticsApi.getAreaSentimentAnalysis(areaName, selectedCity.value.id)
      if (data) {
        // 更新情感分析图表
  ...
  // 加载特定商圈的趋势数据
  const loadAreaTrendData = async (areaName) => {
    try {
      const data = await analyticsApi.getAreaConsumptionTrend(areaName, selectedCity.value.id)
      if (data) {
        // 更新趋势图表
  ...
  // 加载特定商圈的雷达对比数据
  const loadAreaRadarData = async (areaName) => {
    try {
      const data = await analyticsApi.getAreaRadarComparisonData([areaName], selectedCity.value.id)
      if (data) {
        // 更新雷达图表
  ...
  const loadRadarData = async () => {
    ...
    try {
      ...
      const areaNames = hotAreas.map(area => area.name)

      const data = await analyticsApi.getRadarComparisonData(areaNames, selectedCity.value.id)
...
.dashboard-container {
  ...
  height: 100vh;
  background: linear-gradient(135deg, #0a1219 0%, #1a2b4a 100%);
  color: #fff;
}
...
:deep(header) {
  ...
  background: linear-gradient(135deg, 
    ...
    rgba(45, 95, 165, 0.95) 50%,
    rgba(35, 75, 140, 0.95) 75%,
    rgba(25, 50, 105, 0.95) 100%);
...
.center-map {
  ...
  display: flex;
  flex-direction: column;
  background: linear-gradient(135deg, rgba(10, 18, 25, 0.4) 0%, rgba(26, 43, 74, 0.4) 100%);
  border-radius: 12px;
...
.map-controls {
  height: 60px;
  background: linear-gradient(90deg, rgba(30, 60, 114, 0.6) 0%, rgba(42, 82, 152, 0.5) 100%);
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 0 20px;
...
.map-legend {
  ...
  gap: 30px;
  background: linear-gradient(90deg, rgba(30, 60, 114, 0.4) 0%, rgba(42, 82, 152, 0.3) 100%);
...
@media (min-width: 1920px) and (max-width: 2559px) {
  .dashboard-header {
    ...
    box-shadow: 
      ...
      0 3px 18px rgba(30, 60, 114, 0.35),
